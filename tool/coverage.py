...
def sam(bamfile, outdir, prefix):
    sort = 'samtools sort -o {outdir}/coverage/{prefix}.sorted.bam {bam} &&'
    index = 'samtools index {outdir}/coverage/{prefix}.sorted.bam &&'
    bedcovu = 'samtools bedcov {outdir}/coverage/{prefix}_unmappedregions.bed {outdir}/coverage/{prefix}.sorted.bam > {outdir}/coverage/{prefix}_unmapcvg.tsv &&'
    bedcovm = 'samtools bedcov {outdir}/coverage/{prefix}_mappedregions.bed {outdir}/coverage/{prefix}.sorted.bam > {outdir}/coverage/{prefix}_mapcvg.tsv'
    cmd = sort+index+bedcovu+bedcovm
    cmd = cmd.format(prefix = prefix, outdir = outdir, bam = bamfile)
...
def cvg_main(mappedlocations, unmappedlocations, conflictlocations, reverselocations, bamfile, reference, outdir, prefix):
    newpath = 'coverage'
